...
def draw_ship(frame: int, player: int) -> Image.Image:
    img = new_rgba(32, 16)
    # hull colors by player
    hulls = [
        (92, 225, 255, 255),   # p1 cyan
        (255, 107, 53, 255),   # p2 orange
        (46, 204, 113, 255),   # p3 green
        (155, 89, 182, 255),   # p4 violet
        (255, 212, 71, 255),   # p5 gold
    ]
    hull = hulls[player % 5]
    wing = (61, 74, 140, 255)
    ...
    fill_rect(img, 4, 1, 6, 2, wing)
    fill_rect(img, 4, 13, 6, 2, wing)
    # cockpit
...
def draw_meteor(size: int, frame: int) -> Image.Image:
    ...
    hot = (255, 107, 53, 255)
    r = size // 2 - 1
    # rotating blob via angular offset
...
def draw_bullet(kind: int) -> Image.Image:
    ...
    if kind == 0:  # standard
        ...
        fill_rect(img, 1, 2, 5, 1, (92, 225, 255, 255))
        fill_rect(img, 1, 5, 5, 1, (92, 225, 255, 255))
...
def draw_powerup(kind: int) -> Image.Image:
    img = new_rgba(16, 16)
    colors = {
        0: (92, 225, 255, 255),   # shield
        1: (155, 89, 182, 255),   # slow
        2: (255, 212, 71, 255),   # clear
        3: (255, 107, 53, 255),   # weapon upgrade
        4: (46, 204, 113, 255),   # life
    }
    c = colors[kind]
    ellipse(img, 8, 8, 6, 6, c)
    ellipse(img, 8, 8, 4, 4, (10, 10, 24, 255))
    if kind == 0:
        ellipse(img, 8, 8, 3, 3, c, fill=False)
        ellipse(img, 8, 8, 5, 5, c, fill=False)
    elif kind == 1:
        fill_rect(img, 5, 7, 6, 2, c)
        fill_rect(img, 7, 5, 2, 6, c)
    elif kind == 2:
        for i in range(8):
            ang = i * math.pi / 4
            put(img, int(8 + 4 * math.cos(ang)), int(8 + 4 * math.sin(ang)), c)
        ...
        # heart-ish
        fill_rect(img, 5, 6, 2, 2, c)
        fill_rect(img, 9, 6, 2, 2, c)
...
def draw_heart() -> Image.Image:
    img = new_rgba(8, 8)
    c = (196, 30, 58, 255)
    fill_rect(img, 1, 2, 2, 2, c)
    fill_rect(img, 4, 2, 2, 2, c)
    fill_rect(img, 1, 3, 5, 2, c)
    fill_rect(img, 2, 5, 3, 2, c)
    put(img, 3, 7, c)
    return img
...
def draw_button(label_pixels, selected: bool) -> Image.Image:
    ...
    fill_rect(img, 0, 15, 32, 1, border)
    fill_rect(img, 0, 0, 1, 16, border)
    fill_rect(img, 31, 0, 1, 16, border)
    if selected:
...
def draw_big_title_strip() -> Image.Image:
    ...
    for ang in range(-60, 61, 2):
        ...
        for r in range(12, 18):
            put(img, int(32 + math.cos(rad) * r), int(32 + math.sin(rad) * r), (92, 225, 255, 255))
    # star burst
    for i in range(8):
        ang = i * math.pi / 4
        for r in range(4, 10):
...
def draw_weapon_icon(kind: int) -> Image.Image:
    ...
    if kind == 0:
        ...
        fill_rect(img, 3, 5, 10, 2, (92, 225, 255, 255))
        fill_rect(img, 3, 9, 10, 2, (92, 225, 255, 255))
    else:
...
def main():
    ...
    # Ships: 5 players x 2 engine frames -> width 32*5=160, height 32 (2 frames)
